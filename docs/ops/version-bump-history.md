# app/version.py bump history

`app/version.py`'s module docstring used to carry the full rationale for
every version bump inline. That's the right transparency (every deploy that
changes behaviour bumps `__version__`, and the *why* should be discoverable
next to the number) but it made the file itself a recurring victim of the
600-line `pyfile-size-check` CI gate — issue #357's fix tripped it simply by
adding one more bump entry to an already-599-line file. Extracted here so
`app/version.py` stays a thin, gate-safe module forever; new bumps append to
this file, not the module docstring.

`app/version.py` docstring keeps only the *current* bump's one-liner + a
pointer to this file. Full log below, oldest first.
...
fix/issue-357 (0.9.51): tests/_app_factory.py had drifted from
    app.main.create_app(): 14 routers create_app() mounts were missing from
    build_test_app()'s _ROUTER_SPECS entirely (mesh_routes,
    mesh_wellknown_routes, sse_routes, bundle_converge_routes,
    loop_pack_routes, federation_filter_routes, federation_propose_routes,
    connect_test_routes, credits_routes, demand_routes, internal_routes,
    feedback_status_routes, bootcamp_routes, marketing_routes.wisechef_router),
    and personality_routes had its /api prefix DOUBLE-applied (the router
    already bakes in prefix="/api/personalities"; the factory spec re-passed
    prefix="/api" on top of it) — any factory-based test hitting
    /api/personalities/* was silently probing a nonexistent
    /api/api/personalities/* path instead of the real route. New
    tests/test_app_factory_parity.py asserts route-set equality (method,
    path) between create_app() and build_test_app() so this class of drift
    fails loudly instead of silently blinding future factory-based tests to
    routes that don't actually exist. Same PR: extracted this changelog out
    of app/version.py's module docstring into this file — the docstring had
    grown to exactly 600 lines and every future bump was one line from
    tripping the pyfile-size-check gate on a module whose own job is
    tracking behaviour changes, the silliest possible way to fail that gate.
    Test-only change; bump keeps the healthz cutover-proof invariant intact.

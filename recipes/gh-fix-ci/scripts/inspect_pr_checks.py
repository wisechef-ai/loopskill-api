...
def fetch_checks(pr_value: str, repo_root: Path) -> list[dict[str, Any]] | None:
    ...
    if result.returncode != 0:
        ...
        # gh < 2.55 doesn't support `--json` on `pr checks` at all; fall back
        # to the plain-text TSV output and parse it. Format is:
        #   <name>\t<bucket/state>\t<duration>\t<details_url>\t<workflow?>
        if "unknown flag: --json" in message or "Unknown flag --json" in message:
            text_result = run_gh_command(
                ["pr", "checks", pr_value],
                cwd=repo_root,
            )
            if text_result.returncode != 0:
                text_msg = (text_result.stderr or text_result.stdout or "").strip()
                print(text_msg or "Error: gh pr checks failed.", file=sys.stderr)
                return None
            checks: list[dict[str, Any]] = []
            for line in (text_result.stdout or "").splitlines():
                parts = line.split("\t")
                ...
                checks.append({
                    ...
                    "bucket": parts[1].strip() if len(parts) > 1 else "",

#!/usr/bin/env python3
"""Seed the "Ripwire" bundle from the github-ripwire tap.

Composes ONE curated public bundle ("Ripwire") owned by the WiseChef Editorial
SYSTEM account, holding every skill from Red Hat Emerging Technologies'
Apache-2.0 ripwire pack (``redhat-et/ripwire``, tap source id ``github-ripwire``).

These 17 skills are the agent-facing half of the ripwire CLI/MCP server — a
zero-dependency C++23 tool that hands a coding agent a ranked symbol map, a
blast radius, and a tests-to-run list WITHOUT the agent reading the repo. They
are useless in isolation and excellent together (orient -> navigate ->
before-you-build -> change-check), which is exactly what a bundle is for: one
install puts the whole loop in the agent's hands.
...
pipeline and have no internal Skill row until materialized. So this seed:
  ...
  1. Walks the live tap (``github_tap_fetch('github-ripwire')``) to discover the
     real skill dirs — never a hard-coded list, so it tracks upstream as the repo
     adds/removes skills.
...
DESIGN INVARIANTS (mirror seed_marketing_bundle.py):
...
so ~17 GitHub raw fetches at SEED time (not per request). Transient failures are
skipped + reported, so a partial GitHub outage yields a partial bundle you can
top up by re-running — never a crash, never a fabricated membership.

Run on prod:
    cd /home/wisechef/loopskill-api && ./venv/bin/python scripts/seed_ripwire_bundle.py
...
TAP_SOURCE = "github-ripwire"
BUNDLE_SLUG = "ripwire"
BUNDLE_NAME = "Ripwire"
BUNDLE_DESC = (
    "Stop making your agent read the whole repo. Ripwire's 17 skills wire an "
    "agent into a ranked symbol map, a blast radius, and a tests-to-run list "
    "before it writes a line — orient cold in an unfamiliar codebase, navigate "
    "to the right symbol, check what a change actually touches, and hand off "
    "with the context intact.\n\n"
    "Skills by Red Hat Emerging Technologies (redhat-et/ripwire), Apache-2.0 "
    "licensed. Surfaced live from origin — never rehosted."
)
...
def _discover_tap_skill_slugs() -> list[str]:
    ...
    Slugs are the adapter's namespaced form ``github-ripwire--<name>``. Empty

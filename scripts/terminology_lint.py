...
BANNED = [
    ...
    # activate_0701 Phase A1: bare ``Loop`` (the old verifier class name) is
    # banned in NEW code. The canonical name is ``Verifier``. Bare ``Loop(``
    # usage without a compat-alias marker signals un-renamed code.  # compat-alias
    # Case-SENSITIVE: matches the class name ``Loop`` but not lowercase
    # ``loop_count``, ``loop_routes``, ``loop_slug`` etc.
...
def main() -> int:
    ...
    if args.diff_base:
        ...
        violations = [(rel, ln, snip) for (rel, ln, snip) in violations if ln in changed.get(rel, set())]
        if not violations:
            print(f"terminology-lint (diff vs {args.diff_base}): no banned nouns in added lines")
            return 0
    if violations:

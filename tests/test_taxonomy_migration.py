...
def _load_migration():
    ...
    path = Path(__file__).parent.parent / "alembic" / "versions" / "b3c4d5e6f701_v7_phase_f_taxonomy.py"
...
def _run_upgrade(engine):
    ...
    with engine.begin() as conn:
        ...
        conn.execute(text("UPDATE users SET subscription_tier='operator' WHERE subscription_tier='studio'"))
...
def test_mapping_table_matches_docs():
    ...
    mapping_rows = [(o, n) for o, n in pairs if o not in tier_values and n not in tier_values]
    ...
    for old, new in mapping_rows:
        ...
        assert CATEGORY_MAP[old] == new, f"docs maps {old}→{new} but migration maps {old}→{CATEGORY_MAP[old]}"

...
These tests deliberately avoid ``importlib.reload(app.config)`` — reloading
swaps the module-global ``settings`` singleton out from under any test running
concurrently on another xdist worker that monkeypatched it. We monkeypatch the
field + env vars in place instead, which is parallel-safe.
"""
...
def clean_origin_env(monkeypatch):
    """Strip every public-origin env var and pin PUBLIC_ORIGIN empty so we test
    the true default — in place, no module reload."""
    for var in (
        "WR_PUBLIC_ORIGIN",
    ...
    monkeypatch.setattr(settings, "PUBLIC_ORIGIN", "", raising=False)
    yield
...
def test_public_origin_primary_env_is_loopskill(clean_origin_env, monkeypatch):
    """LOOPSKILL_PUBLIC_ORIGIN is the primary env override."""
    monkeypatch.setenv("LOOPSKILL_PUBLIC_ORIGIN", "https://skills.example.com")
    assert config.public_origin() == "https://skills.example.com"
...
def test_public_origin_wr_field_takes_precedence(clean_origin_env, monkeypatch):
    ...
    monkeypatch.setattr(settings, "PUBLIC_ORIGIN", "https://primary.example.com", raising=False)
    monkeypatch.setenv("LOOPSKILL_PUBLIC_ORIGIN", "https://secondary.example.com")
    assert config.public_origin() == "https://primary.example.com"
...
def test_public_origin_recipes_env_still_honored_as_compat(clean_origin_env, monkeypatch):
    ...
    assert config.public_origin() == "https://legacy.example.com"


def test_public_origin_loopskill_beats_recipes_compat(clean_origin_env, monkeypatch):
    """When both new and compat env vars are set, the new one wins."""
    monkeypatch.setenv("LOOPSKILL_PUBLIC_ORIGIN", "https://new.example.com")
    monkeypatch.setenv("RECIPES_PUBLIC_ORIGIN", "https://old.example.com")
    assert config.public_origin() == "https://new.example.com"

...
def test_wikipedia_citation_domains_pass() -> None:
    ...
    Regression: `humanizer` cites Wikipedia:Signs of AI writing as the source
    its whole method derives from, and was rejected at publish time (HTTP 422)
    on 2026-08-10 because wikipedia.org was absent from the allowlist -- while
    arxiv.org and semanticscholar.org, the same *kind* of citation source, were
    already present. A citation is not promotion.
    """
    body = CLEAN_SKILL_MD + (

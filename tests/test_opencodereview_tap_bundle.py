"""github-opencodereview tap + first-class metasearch + Open Code Review bundle.

Covers the surfaces the alibaba/open-code-review pack ingestion touches:
  1. github_taps.py        : the github-opencodereview tap entry + in_metasearch.
  2. metasearch_fanout     : DEFAULT_FANOUT_SOURCES includes the tap (the "no
                             external ghetto" wiring) + a real fan-out run
                             surfacing a github-opencodereview skill first-class.
  3. seed_opencodereview_bundle : discovers tap skills live, materializes them
                             as federated pointers, composes the public
                             "Open Code Review" bundle with Apache-2.0
...
class TestOpenCodeReviewTap:
    def test_tap_entry_present_and_trusted(self):
        from app.services.github_taps import TAP_BY_SOURCE

        tap = TAP_BY_SOURCE.get("github-opencodereview")
        assert tap is not None, "github-opencodereview tap must be registered"
    ...
    def test_tap_registered_in_live_sources(self):
        ...
        assert "github-opencodereview" in LIVE_SOURCES

    def test_adapter_resolves_ocr_tap(self):
        from app.services.federation_adapters import GitHubTapAdapter, get_adapter

        ad = get_adapter("github-opencodereview", fetch=lambda q: [])
        assert isinstance(ad, GitHubTapAdapter)
        assert ad.source_id == "github-opencodereview"

    def test_source_is_a_github_facet(self):
        from app.services.github_taps import GITHUB_FACET_SOURCES

        assert "github-opencodereview" in GITHUB_FACET_SOURCES
...
class TestMetasearchInclusion:
    def test_fanout_default_sources_include_ocr_tap(self):
        from app.services.metasearch_fanout import DEFAULT_FANOUT_SOURCES

        assert "github-opencodereview" in DEFAULT_FANOUT_SOURCES, (
            "github-opencodereview must ride the first-class fan-out, not the legacy /external ghetto"
        )
    ...
    def test_metasearch_tap_sources_derives_from_flag(self):
        ...
        assert "github-opencodereview" in METASEARCH_TAP_SOURCES
        for src in METASEARCH_TAP_SOURCES:
            assert TAP_BY_SOURCE[src].in_metasearch is True

    def test_fanout_surfaces_ocr_skill_first_class(self, monkeypatch):
        """A github-opencodereview skill flows through fan_out → first-class pair."""
        ...
        rl.reset_all()
        row = {
            "slug": "github-opencodereview--open-code-review",
            "name": "open-code-review",
            "description": "AI code review on git diffs",
            "license": "Apache-2.0",
            "redistributable": True,
            "html_url": f"https://github.com/{REPO}",
        }
        monkeypatch.setitem(fl.LIVE_FETCH, "github-opencodereview", lambda q: [row])
        out = fo.fan_out("code review", sources=("github-opencodereview",))
        rl.reset_all()
        assert "github-opencodereview" in out.sources_ok
        assert len(out.pairs) == 1
        skill, _raw = out.pairs[0]
        assert skill.source == "github-opencodereview"
...
class TestBundleCap:
    def test_cap_fits_the_ocr_pack(self):
        from app.bundle_routes import BUNDLE_SKILL_CAP

        assert BUNDLE_SKILL_CAP >= 2, "cap must fit the full open-code-review pack"
...
def _stub_ocr_ext(source: str, slug: str) -> ExternalSkill:
    leaf = slug.split("--", 1)[-1]
    return ExternalSkill(
        ...
        license="Apache-2.0",
        redistributable=True,
        description=f"Open Code Review skill: {leaf}",
    )


def _drive(seed, monkeypatch, db_session, slugs, resolver=_stub_ocr_ext):
...
class TestSeedOpenCodeReviewBundle:
    def test_seed_composes_public_bundle_with_apache_attribution(self, db_session, monkeypatch):
        ...
        tap_slugs = [
            "github-opencodereview--open-code-review",
            "github-opencodereview--open-code-review-delegate",
            "github-opencodereview--ocr-third",
        ]
        _drive(seed, monkeypatch, db_session, tap_slugs)

        assert seed.seed(dry_run=False) == 0

        cb = db_session.query(Bundle).filter(Bundle.slug == "open-code-review").first()
        ...
        assert "Alibaba" in cb.description
        ...
        for sk in skills:
            ...
            assert sk.slug.startswith("ext:github-opencodereview:")
    ...
    def test_seed_is_idempotent(self, db_session, monkeypatch):
        ...
        _drive(
            ...
            monkeypatch,
            db_session,
            ["github-opencodereview--open-code-review", "github-opencodereview--open-code-review-delegate"],
        )
        assert seed.seed(dry_run=False) == 0
    ...
    def test_seed_aborts_on_partial_failure_without_allow_partial(self, db_session, monkeypatch):
        ...
        def _flaky(source, slug):
            return None if slug.endswith("broken") else _stub_ocr_ext(source, slug)

        _drive(
            seed,
            monkeypatch,
            db_session,
            ["github-opencodereview--open-code-review", "github-opencodereview--broken"],
            resolver=_flaky,
        )
    ...
    def test_seed_reconciles_removed_skills_off(self, db_session, monkeypatch):
        ...
        both = ["github-opencodereview--open-code-review", "github-opencodereview--open-code-review-delegate"]
    ...
    def test_dry_run_writes_nothing(self, db_session, monkeypatch):
        ...
        assert db_session.query(Bundle).filter(Bundle.slug == "open-code-review").first() is None
...
class TestTrackedBundleAutoTrack:
    def test_ocr_is_a_tracked_bundle(self):
        import scripts.federation_reindex as fr

        tracked = {t.source: t for t in fr.TRACKED_BUNDLES}
        assert "github-opencodereview" in tracked
        assert tracked["github-opencodereview"].seed_module == "scripts.seed_opencodereview_bundle"
        # The table must not have dropped the pre-existing tracked bundles.
    ...
    def test_tap_ok_gate_is_per_source(self):
        ...
        reports = [
            {"source": "github-opencodereview", "status": "ok", "indexed": 2},
            {"source": "github-marketing", "status": "error", "indexed": None},
        ]
        assert _tap_ok(reports, "github-opencodereview") is True
    ...
    def test_reconcile_is_non_fatal(self, monkeypatch):
        ...
        monkeypatch.setattr("scripts.seed_opencodereview_bundle.seed", _boom)
        tracked = next(t for t in fr.TRACKED_BUNDLES if t.source == "github-opencodereview")
    ...
    def test_reindex_main_triggers_reconcile_after_successful_walk(self, monkeypatch):
        ...
        monkeypatch.setattr("app.services.federation.LIVE_SOURCES", ["github-opencodereview"], raising=False)
        monkeypatch.setattr(
            "sys.argv", ["federation_reindex.py", "--source", "github-opencodereview", "--dry-run"]
        )
        assert fr.main() == 0
        assert calls.get("github-opencodereview") is True, (
            "reconcile must run after a successful open-code-review walk"
        )

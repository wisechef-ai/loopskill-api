...
class TestListDetail:
    def test_list_only_returns_mine(self, db_session):
        ...
        cb_mine = Bundle(id=uuid4(), name="Mine", bundle_owner=mine.id)
        cb_other = Bundle(id=uuid4(), name="Other", bundle_owner=other.id)
    ...
    def test_get_detail_includes_skills(self, db_session):
        user = _make_user(db_session, tier="pro_plus")
        cb = Bundle(id=uuid4(), name="Mine", bundle_owner=user.id)
    ...
    def test_get_other_users_cookbook_returns_404(self, db_session):
        ...
        cb = Bundle(id=uuid4(), name="Private", bundle_owner=owner.id)
        db_session.add(cb)
        db_session.commit()
    ...
    def test_get_detail_enriched_fields_for_viz(self, db_session):
        ...
        user = _make_user(db_session, tier="pro_plus")
        cb = Bundle(id=uuid4(), name="Viz CB", bundle_owner=user.id)
        db_session.add(cb)
        db_session.flush()
...
class TestManifest:
    def test_manifest_yaml_roundtrip(self, db_session):
        user = _make_user(db_session, tier="pro_plus")
        cb = Bundle(id=uuid4(), name="Manifest CB", description="My desc", bundle_owner=user.id)
...
class TestInstall:
    ...
    def test_install_skips_disabled(self, db_session):
        ...
        db_session.add_all(
            [
                BundleSkill(bundle_id=cb.id, skill_id=kept.id, source="custom-added"),
                BundleSkill(bundle_id=cb.id, skill_id=gone.id, source="disabled"),
            ]
        )
        db_session.commit()
...
class TestSync:
    ...
    def test_sync_partitions_by_source(self, db_session):
        ...
        db_session.add_all(
            [
                BundleSkill(bundle_id=cb.id, skill_id=a.id, source="custom-added"),
                BundleSkill(bundle_id=cb.id, skill_id=u.id, source="overridden", pinned_version="2.0.0"),
                BundleSkill(bundle_id=cb.id, skill_id=r_skill.id, source="disabled"),
            ]
        )
        db_session.commit()

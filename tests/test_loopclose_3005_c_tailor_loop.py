...
loopskill_tailor_version (upload a version, base64 transport) and
loopskill_bundle_attach (bridge a fork's latest version into a cookbook as a
real installable Skill) — closing the loop:

    loopskill_tailor -> loopskill_tailor_version -> loopskill_bundle_attach
                   -> loopskill_bundle_install

Covers:
  Tier gate (Pro+ required; master/free/None rejected) for both tools.
  loopskill_tailor_version: happy path, invalid semver, invalid base64, empty,
    fork-not-found, duplicate-semver, latest-pointer advance.
  loopskill_bundle_attach: happy path (promote + mint SkillVersion), no-version
...
class TestDogfoodRoundTrip:
    def test_tailor_to_install(self, db: Session, storage_dirs) -> None:
        ...
        # 1. tailor (fork)
        t = loopskill_tailor(db, source_slug="round-trip-src", name="Round Trip Fork", ctx=ctx)
        ...
        a = loopskill_bundle_attach(db, fork_id=fork_id, target_cookbook_id=str(cb.id), ctx=ctx)
        ...
        inst = loopskill_bundle_install(db=db, ctx=ctx, cookbook_id=str(cb.id), slug=promoted_slug)
    ...
    def test_salt_parity(self, db: Session, storage_dirs) -> None:
        ...
        fork_id = loopskill_tailor(db, source_slug="salt-src", name="Salt Fork", ctx=ctx)["fork_id"]
        loopskill_tailor_version(db, fork_id=fork_id, tarball_base64=_b64_tarball(), semver="1.2.3", ctx=ctx)
        a = loopskill_bundle_attach(db, fork_id=fork_id, target_cookbook_id=str(cb.id), ctx=ctx)
        inst = loopskill_bundle_install(db=db, ctx=ctx, cookbook_id=str(cb.id), slug=a["skill_slug"])
...
class TestMCPDispatch:
    def test_tools_registered(self) -> None:
        ...
        assert "loopskill_tailor_version" in names
        assert "loopskill_bundle_attach" in names

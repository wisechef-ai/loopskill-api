...
def test_external_skill_deploys_to_fleet_with_pin(db_session, monkeypatch, tmp_path):
    ...
    assert cs.pinned_version == body["pinned_semver"], "pinned to the deploy-time content SHA"
...
def test_redeploy_advances_pin(db_session, monkeypatch, tmp_path):
    _mock_resolvable(monkeypatch, body="---\nname: x\n---\n# v1", tmp_path=tmp_path)
    user = _make_user(db_session)
...
def test_deploy_to_unowned_fleet_rejected(db_session, monkeypatch, tmp_path):
    _mock_resolvable(monkeypatch, tmp_path=tmp_path)
...
def test_reactivating_disabled_row_respects_cap(db_session, monkeypatch, tmp_path):
    ...
    # first deploy the external skill, then DISABLE its row
    pin = pin_external_for_deploy(db_session, "skills-sh", "o--r--s")
    from app.models import Skill

    ext_skill_id = db_session.query(Skill).filter(Skill.slug == "ext:skills-sh:o--r--s").first().id
    db_session.add(
        BundleSkill(
            bundle_id=cb.id, skill_id=ext_skill_id, source="disabled", pinned_version=pin.pinned_semver
        )
    )
    ...
    for i in range(BUNDLE_SKILL_CAP):
        ...
        db_session.add(s)
        db_session.flush()
        db_session.add(BundleSkill(bundle_id=cb.id, skill_id=s.id, source="custom-added"))
    db_session.commit()
    ...
    assert r.status_code == 403, "reactivating a disabled row at cap must be rejected"
    assert r.json()["detail"]["reason"] == "skill_cap_reached"

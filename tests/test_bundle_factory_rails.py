...
def _skill(slug: str, *, tier="free", public=True, archived=False) -> dict:
    return {
        "slug": slug, "title": slug, "description": f"{slug} does things",
        "tier": tier, "is_public": public, "is_archived": archived,
    }
...
def test_federated_member_resolves_via_normalization(patch_get):
    ...
    fed = {"launch-strategy": {"total": 1, "results": [
    ...
    patch_get(Router(
        index={"fedbundle": {"skills": [{"name": ident, "dir_name": ident, "files": ["SKILL.md"]},
...
def test_federated_member_zero_hits_fails(patch_get):
    ...
    patch_get(Router(
        index={"feddead": {"skills": [{"name": ident, "dir_name": ident, "files": ["SKILL.md"]},
                                       *_index(["local-one"])["skills"]]}},
...
def _cat_skills(cat: str, n: int) -> list[dict]:
    return [
        {"slug": f"{cat}-{i}", "title": f"{cat} helper {i}",
         "description": f"{cat} tooling task number {i}", "category": cat,
         "tier": "free", "is_public": True, "is_archived": None,
...
def test_candidates_only_free_public(monkeypatch, tmp_path):
    rows = _cat_skills("ops", 4) + [
        {"slug": "pro-thing", "title": "pro thing", "description": "ops tooling task",
         "category": "ops", "tier": "pro", "is_public": True, "is_archived": None},
        {"slug": "arch-thing", "title": "arch thing", "description": "ops tooling task",
         "category": "ops", "tier": "free", "is_public": True, "is_archived": True},
    ]
...
def test_candidates_never_below_three_members(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(bc.time, "sleep", lambda *_: None)
    out = tmp_path / "c.json"
    rc = bc.main(["--base", BASE, "--out", str(out)])

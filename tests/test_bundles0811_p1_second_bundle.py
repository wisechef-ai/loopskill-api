...
def _load_backfill_migration():
    path = (
        ...
        / "alembic"
        / "versions"
        / "bundles0811_p1_slug_backfill.py"
    )
...
def test_migration_backfills_the_one_null_slug_public_row(db_session):
    ...
    private_null_id = _insert_legacy_bundle_row(
        db_session, name="Private No Slug", visibility="private"
    )
    db_session.commit()
    ...
    assert (
        db_session.query(Bundle).filter(Bundle.id == other_id).first().slug
        == "other-public-bundle"
    )
...
def test_migration_collision_suffix_matches_route_level_discipline(db_session):
    ...
    assert (
        db_session.query(Bundle).filter(Bundle.id == existing_id).first().slug == "agent-marketing"
    )
...
def test_anonymous_fork_preview_succeeds_no_auth_no_write(app_client, db_session):
    ...
    assert db_session.query(Bundle).count() == before_count, (
        "preview must be a pure read — no new bundle row"
    )
...
def test_fork_claim_401s_anonymously_with_same_token_echoed_in_the_retry_contract(
...
def test_fork_claim_succeeds_once_authenticated_and_copies_active_members(
    app_client, db_session
):
    ...
    db_session.add(
        BundleSkill(bundle_id=bundle.id, skill_id=disabled_skill.id, source="disabled")
    )
    db_session.commit()
    ...
    assert new_bundle.visibility == "private"  # born private — claiming costs an account, not "public by default"
    copied_members = (
        db_session.query(BundleSkill).filter(BundleSkill.bundle_id == new_bundle.id).all()
    )
...
def test_fork_claim_rejects_a_bundle_that_went_private_between_preview_and_claim(
    app_client, db_session
):
...
def test_public_bundle_page_install_command_resolves_against_a_working_anonymous_route(
    ...
    assert (
        install_command
        == f"curl -fsSL {public_origin()}/api/bundles/install.sh | bash -s -- {bundle.slug}"
    )
    ...
    match = re.search(r'INDEX_URL="\$API_BASE(/api/bundles/public/\$SLUG/\.well-known/skills/index\.json)"', script_resp.text)
...
def test_install_command_reports_locked_paid_members_by_name_not_silently(
    app_client, db_session
):
    ...
    index_resp = app_client.get(
        f"/api/bundles/public/{bundle.slug}/.well-known/skills/index.json"
    )

...
def test_compare_hard_incompat_arch():
    fp = probe_mod.HostFingerprint(os="linux", arch="x86_64", ram_gb=8, disk_gb=20, network=True)
    res = probe_mod.compare(fp, {"os": ["linux"], "arch": ["arm64"], "ram_gb": 4, "network": "required"})
    assert res.decision == "hard_incompat"
    assert any("arch" in h for h in res.hard)


def test_compare_soft_incompat_gpu_preferred():
    fp = probe_mod.HostFingerprint(
        os="linux", arch="x86_64", ram_gb=8, disk_gb=20, has_gpu=False, network=True
    )
    res = probe_mod.compare(
        fp,
        {
            "os": ["linux"],
            "arch": ["x86_64"],
...
def test_compare_ram_minimum_violation():
    fp = probe_mod.HostFingerprint(os="linux", arch="x86_64", ram_gb=2, disk_gb=20, network=True)
    res = probe_mod.compare(
        fp,
        {
            "os": ["linux"],
...
def test_compare_ok():
    fp = probe_mod.HostFingerprint(os="linux", arch="x86_64", ram_gb=8, disk_gb=20, network=True)
    res = probe_mod.compare(fp, {"os": ["linux"], "arch": ["x86_64"], "ram_gb": 4, "network": "required"})

...
def like_track(slug: str, request: Request, db: Session = Depends(get_db)):
    ...
    #
    # spotify_2607 Phase A (plan §0 decision #3 / §0b L6 supersession): a
    # FEDERATED like now ALSO lands in the deployable Liked bundle. The prior
    # ponytail_0724 lock kept federated likes engagement-only because
    # BundleSkill drives authz.can_install — but 76% of the catalog is
    # federated, so a Liked bundle that drops 3-in-4 saves is worse than
    # useless. This override is RECORDED, not silent: Phase B/C ship badging +
    # a vetted/community install-payload split that make it defensible.
    #
    ...
    # it self-heals rows created before this shipped (Adam's 2 orphaned likes
    # repair on next interaction, and the one-shot backfill places them without
    # any user action — see scripts/spotify_2607_a_backfill_liked_federated.py).
    if skill_id is not None:
...
def unlike_track(slug: str, request: Request, db: Session = Depends(get_db)):
    ...
    # spotify_2607 Phase A: extended to the FEDERATED branch (mirrors the like
    # path) — an unlike must remove the federated row from the deployable
    # bundle in the SAME transaction as the skill_likes delete (proven by a
    # mid-flow-kill test).
    if skill_id is not None:
        skill_row = db.execute(select(Skill).where(Skill.id == skill_id)).scalar_one_or_none()
        if skill_row is not None:
